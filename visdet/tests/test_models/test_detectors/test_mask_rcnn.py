...
def test_mask_rcnn_init():
    ...
    with DefaultScope.overwrite_default_scope("visdet"):
        ...
        cfg = dict(
            ...
            backbone=dict(
                type="SwinTransformer", embed_dims=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], window_size=7, out_indices=(0, 1, 2, 3)
            ),
            ...
            roi_head=dict(
                ...
                bbox_roi_extractor=dict(
                    type="SingleRoIExtractor", roi_layer=dict(type="RoIAlign", output_size=7), out_channels=256, featmap_strides=[4, 8, 16, 32]
                ),
                bbox_head=dict(
                ...
                mask_roi_extractor=dict(
                    type="SingleRoIExtractor", roi_layer=dict(type="RoIAlign", output_size=14), out_channels=256, featmap_strides=[4, 8, 16, 32]
                ),
                mask_head=dict(
                    type="FCNMaskHead",
                    num_convs=4,
...
def test_mask_rcnn_forward():
    ...
    with DefaultScope.overwrite_default_scope("visdet"):
        ...
        batch_data_samples[0].set_metainfo(
            {"img_shape": (224, 224), "ori_shape": (224, 224), "pad_shape": (224, 224), "batch_input_shape": (224, 224), "scale_factor": (1.0, 1.0)}
        )

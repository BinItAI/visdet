...
from visdet.engine.fileio import get_file_backend, isdir, join_path, list_dir_or_file, load
from visdet.engine.logging import print_log
from visdet.engine.registry import FUNCTIONS, MODELS, VISUALIZERS, DefaultScope
from visdet.engine.runner.checkpoint import _load_checkpoint, _load_checkpoint_to_model
...
class BaseInferencer(metaclass=InferencerMeta):
    ...
    def _get_models_from_metafile(dir: str):
        ...
        if not osp.exists(meta_index_path):
            raise FileNotFoundError(
                f"Cannot find {meta_index_path}. The model-index/metafile system has been removed; "
    ...
    def list_models(scope: str | None = None, patterns: str = r".*"):
        ...
        if not osp.exists(meta_index_path):
            if scope == "visdet":
                print_log(
                    "model-index/metafile model discovery is disabled in visdet; "
                    "use task-specific inferencers or YAML presets.",
                    logger="current",
                )
                return []

...
from visdet.cv.cnn.bricks.norm import is_norm
from visdet.engine.model import bias_init_with_prob, constant_init, normal_init
...
from visdet.models.utils import images_to_levels, inverse_sigmoid, multi_apply, sigmoid_geometric_mean, unmap
from visdet.registry import MODELS
from visdet.structures.bbox import distance2bbox
from visdet.utils import ConfigType, InstanceList, OptInstanceList, reduce_mean
...
class RTMDetHead(ATSSHead):
    ...
    def forward(self, feats: tuple[Tensor, ...]) -> tuple[tuple[Tensor, ...], tuple[Tensor, ...]]:
        cls_scores = []
        bbox_preds = []
    ...
    def loss_by_feat_single(
        ...
        loss_cls = self.loss_cls(cls_score, (labels, assign_metrics), label_weights, avg_factor=1.0)
    ...
    def loss_by_feat(
        ...
        if len(featmap_sizes) != self.prior_generator.num_levels:
            raise ValueError("featmap_sizes must match prior_generator.num_levels")
        ...
        for anchor, bbox_pred in zip(anchor_list[0], bbox_preds):
            ...
            decoded_bboxes.append(distance2bbox(anchor, bbox_pred))
        ...
        anchors_list, labels_list, label_weights_list, bbox_targets_list, assign_metrics_list, _sampling_results = (
            cls_reg_targets
        )
        ...
        losses_cls = [loss / cls_avg_factor for loss in losses_cls]

        bbox_avg_factor = reduce_mean(sum(bbox_avg_factors)).clamp_(min=1).item()
        losses_bbox = [loss / bbox_avg_factor for loss in losses_bbox]
    ...
    def get_targets(
        ...
        num_imgs = len(batch_img_metas)
        if len(anchor_list) != len(valid_flag_list) or len(anchor_list) != num_imgs:
            raise ValueError("anchor_list/valid_flag_list must match num_imgs")
        ...
        if any(labels is None for labels in all_labels):
            raise ValueError("No valid anchors inside image boundary")
    ...
    def get_anchors(
        ...
        for img_meta in batch_img_metas:
            valid_flag_list.append(self.prior_generator.valid_flags(featmap_sizes, img_meta["pad_shape"], device))
...
class RTMDetSepBNHead(RTMDetHead):
    ...
    def forward(self, feats: tuple[Tensor, ...]) -> tuple[tuple[Tensor, ...], tuple[Tensor, ...]]:
        ...
        for idx, (x, stride) in enumerate(zip(feats, self.prior_generator.strides)):
            ...
            reg_raw = self.rtm_reg[idx](reg_feat)
            reg_dist = reg_raw.exp() * stride[0] if self.exp_on_reg else reg_raw * stride[0]

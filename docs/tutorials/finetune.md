...
This tutorial provides instruction for users to use the models provided in the [Model Zoo](../model-zoo.md) for other datasets to obtain better performance.
...
To release the burden and reduce bugs in writing the whole configs, this framework support inheriting configs from multiple existing configs. To finetune a Mask RCNN model, the new config needs to inherit
...
The users may also need to prepare the dataset and write the configs about dataset. this framework already support VOC, WIDER FACE, COCO and Cityscapes Dataset.

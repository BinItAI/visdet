...
def test_libra_rcnn_forward_backward():
    model_cfg = dict(
        type="FasterRCNN",
        backbone=dict(
            type="ResNet",
        ...
        neck=[
            dict(type="FPN", in_channels=[64, 128, 256, 512], out_channels=256, num_outs=5),
            dict(type="BFP", in_channels=256, num_levels=5, refine_level=2, refine_type="conv"),
        ],
        rpn_head=dict(
            type="RPNHead",
            in_channels=256,
        ...
        train_cfg=dict(
            ...
            rcnn=dict(
                ...
                sampler=dict(
                    type="IoUBalancedNegSampler", num=512, pos_fraction=0.25, floor_thr=-1, floor_fraction=0, num_bins=3
                ),
                pos_weight=-1,
                debug=False,
            ),
    ...
    print("Libra R-CNN forward/backward passed!")


if __name__ == "__main__":
    test_libra_rcnn_forward_backward()

...
def test_ms_rcnn_forward_backward():
    model_cfg = dict(
        type="FasterRCNN",
        backbone=dict(
            type="ResNet",
        ...
        test_cfg=dict(
            ...
            roi=dict(score_thr=0.05, nms=dict(type="nms", iou_threshold=0.5), max_per_img=100, mask_thr_binary=0.5),
        ),
    )
    ...
    for i in range(2):
        ...
        gt_instances.bboxes = torch.tensor([[10, 10, 50, 50], [20, 20, 80, 80]], dtype=torch.float32)
        gt_instances.labels = torch.tensor([1, 2], dtype=torch.long)

        masks = np.zeros((2, 224, 224), dtype=np.uint8)
        masks[0, 10:50, 10:50] = 1
        masks[1, 20:80, 20:80] = 1
        gt_instances.instance_masks = BitmapMasks(masks, 224, 224)
    ...
    assert "loss_mask_iou" in losses
    ...
    print("MS R-CNN forward/backward passed!")


if __name__ == "__main__":
    test_ms_rcnn_forward_backward()

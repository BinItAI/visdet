...
class FSDPStrategy(DDPStrategy):
    ...
    def _wrap_model(self, model: nn.Module) -> None:
        ...
        model.set_state_dict_type(model, self.state_dict_type, self.state_dict_config, self.optim_state_dict_config)
    ...
    def _init_state_dict_cfg(self, state_dict_cfg: str | dict) -> None:
        ...
        if not isinstance(self.optim_state_dict_config, OptimStateDictConfig):
            raise TypeError(f"optim_state_dict_config must be OptimStateDictConfig, but got {type(self.optim_state_dict_config)}")
    ...
    def build_optim_wrapper(
        ...
        if isinstance(optim_wrapper, BaseOptimWrapper):
            ...
            assert not optim_state_dict["state"], "Optimizer state_dict should be empty when giving an built optim_wrapper to FSDPStrategy"
        ...
        if isinstance(optim_wrapper, dict | ConfigDict | Config):
            ...
            if optim_wrapper.get("type", "AmpOptimWrapper") in ("AmpOptimWrapper", AmpOptimWrapper):

...
from visengine.device import is_mlu_available, is_musa_available, is_npu_available
...
def _init_dist_pytorch(backend, init_backend="torch", **kwargs) -> None:
    ...
    if is_mlu_available():
        ...
        if init_backend == "torch":
            ...
            raise ValueError(f'supported "init_backend" is "torch" or "deepspeed", but got {init_backend}')
...
def get_data_device(data: Tensor | Mapping | Iterable) -> torch.device:
    ...
    Examples:
        >>> import torch
        >>> from visengine.dist import cast_data_device
        >>> # data is a Tensor
...
def cast_data_device(
    ...
    if out is not None:
        if type(data) is not type(out):
            raise TypeError(f"out should be the same type with data, but got data is {type(data)} and out is {type(data)}")

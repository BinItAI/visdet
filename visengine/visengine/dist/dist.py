...
import visengine
from visengine.device import is_npu_available
...
def _broadcast_object_list(object_list: list[Any], src: int = 0, group: ProcessGroup | None = None) -> None:
    ...
    if my_rank == src:
        tensor_list, size_list = zip(*[_object_to_tensor(obj) for obj in object_list], strict=False)
...
def all_reduce_dict(data: dict[str, Tensor], op: str = "sum", group: ProcessGroup | None = None) -> None:
    ...
    if world_size > 1:
        ...
        split_tensors = [x.reshape(shape) for x, shape in zip(torch.split(flatten_tensor, tensor_sizes), tensor_shapes, strict=False)]

        for k, v in zip(keys, split_tensors, strict=False):
            data[k] = v


def _all_gather_object(object_list: list[Any], obj: Any, group: ProcessGroup | None = None) -> None:
...
def _validate_output_list_for_rank(my_rank: int, dst: int, gather_list: list | None) -> None:
...
def _gather_object(obj: Any, object_gather_list=None, dst: int = 0, group: ProcessGroup | None = None) -> None:
...
def collect_results_cpu(result_part: list, size: int, tmpdir: str | None = None) -> list | None:
    ...
    if tmpdir is None:
        ...
        if rank == 0:
            visengine.mkdir_or_exist(".dist_test")
        ...
        visengine.mkdir_or_exist(tmpdir)
        ...
        for i in range(world_size):
            ...
            if not osp.exists(path):
                raise FileNotFoundError(
                    f"{tmpdir} is not an shared directory for rank {i}, please make sure {tmpdir} is a shared directory for all ranks!"
                )
